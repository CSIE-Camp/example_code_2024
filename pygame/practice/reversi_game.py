...
def ai_design(board, player, step):
    try:
        from teamW import calculate_weight as calculate_weight_W
        importlib.reload(__import__('teamW'))
    except Exception as e:
        global teamW_name
        print(f"{teamW_name} error!!!")
        print(e)
    try:
        from teamB import calculate_weight as calculate_weight_B
        importlib.reload(__import__('teamB'))
    except Exception as e:
        global teamB_name
        print(f"{teamB_name} error!!!")
        print(e)
    global delay
    if step >= 50:
        delay = 500
        ...
        try:
            ...
            if possible_moves:  # 如果有合法位置
                for move in possible_moves:
                    ...
                    weight = calculate_weight_W(board_copy, row, col, player, step)  # 計算權重
...
def battle_group_set():
    ...
    global teamW_name
    global teamB_name
    teamW_name = input("input teamW program name: ")
    teamB_name = input("input teamB program name: ")
    
    try:
        shutil.copy(f"{teamW_name}_champion.py", "teamW.py")
    except:
        print(f"File {teamW_name}_champion.py not found.")
        return -1
    try:
        shutil.copy(f"{teamB_name}_champion.py", "teamB.py")
    except:
        print(f"File {teamB_name}_champion.py not found.")
        return -1
...
def main():
    ...
    while True:
        ...
        while True:
            # 繪製棋盤
            if(game_mode == 'computer_computer'):
                timer = 0
